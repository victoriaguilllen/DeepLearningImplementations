...
class StepLR(torch.optim.lr_scheduler.LRScheduler):
    ...
    def step(self, epoch: Optional[int] = None) -> None:
        ...
        # compruebo que han pasado los steps necesarios
        if (self.counters % self.step_size == 0) and (self.counters != 1):
            # actualizo el lr de cada grupo deloptimizador
            for group in self.optimizer.param_groups:
                group[
                    "lr"
                ] *= self.gamma  # la operacion es inplace porque empleo el operador *=
